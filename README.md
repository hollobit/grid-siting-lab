...
- **MEGA PROJECT 메뉴**: 전력계획 정합(전원·계통·제도) 카드, 추진 전략 시나리오 4안(초대형 집중/허브-스포크/중소 분산/전원 병행) 장단점·평가 매트릭스와 최적 하이브리드 제안, 1단계(허브 9곳)·2단계(RE100 존·SMR 허브·HVDC 회랑) 지도 표시 버튼, 목표 배분 테이블, 마일스톤 타임라인, 6대 해결 과제
...
**교통망 · Context**에서 **도로망**과 **철도망**을 독립적으로 켭니다. 2D와 같은 OSM 고속도로 20,346개 구간·철도 13,414개 구간(2026-07-15 추출)을 사용하며, 금색 고속도로·일반철도 점선·굵은 고속철도 점선으로 구분합니다. **주변 교통망**은 두 강조 레이어를 켜고 현재 AIDC 주변을 넓게 보여줍니다. 클릭하면 교통망 종류를 확인합니다. 기본은 꺼짐이며, 강조를 꺼도 배경지도 도로·철도는 남습니다. 일반 도로 전체를 포함하지 않으며 교량·터널의 높이·깊이는 지형 표면 경로로 대체됩니다. 스냅샷 연결 전에는 2D 개략 교통축임을 표시하고, 데이터가 도착하면 사용자의 토글 상태를 유지한 채 갱신합니다.

**전력망 · Power grid**는 2D와 같은 OSM 송전선 스냅샷(2026-07-15 추출, 3,652개)을 기본으로 표시합니다. 765 kV는 보라색, 345 kV는 분홍색, 154 kV 이하·기타는 청록색이며, 클릭하면 전압을 확인합니다. **전력망 표시**로 켜고 끄며 **주변 전력망**은 현재 AIDC 주변을 넓게 보여줍니다. 선은 지형 표면의 경로로 철탑·전선의 높이는 구현하지 않습니다. 2D에서 공유한 허브 6곳은 실제 변전소 좌표가 아닌 개념 위치입니다. 스냅샷 미연결 시 개략 선형임을 표시하고, 지연 로드가 완료되면 같은 데이터로 자동 전환합니다. 지도 간 표시 토글은 독립적이며 접속 여유용량·실시간 운영 상태를 나타내지 않습니다.
